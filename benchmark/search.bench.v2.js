...
function searchV2 (searchParams) {
  ...
  if (searchParams.input.length >= 3) {
    result = result.concat(searchByTitleV2(keywords, searchParams.receipts)); // keyword full string
    result = result.concat(searchByDescriptionV2(keywords, searchParams.receipts)); // keyword full string
    result = result.concat(searchByApplianceV2(keywords, searchParams.receipts)); // keyword string one word
    result = result.concat(searchByIngredientsV2(keywords, searchParams.receipts)); // keyword string one word
    result = result.concat(searchByUstensilsV2(keywords, searchParams.receipts)); // keyword string one word
  } else {
...
function searchByTitleV2 (keywords, listReceipts) {
  const keywordsString = keywords.join(' ');

  return listReceipts.filter(item => formatString(item.name).includes(keywordsString) && keywordsString.length >= 3);
}

function searchByDescriptionV2 (keywords, listReceipts) {
  const keywordsString = keywords.join(' ');

  return listReceipts.filter(item => formatString(item.description).includes(keywordsString) && keywordsString.length >= 3);
}

function searchByIngredientsV2 (keywords, listReceipts) {
  let result = [];
...
function searchByApplianceV2 (keywords, listReceipts) {
  const keywordsString = keywords.join(' ');

  return listReceipts.filter(item => formatString(item.appliance).includes(formatString(keywordsString)) && keywordsString.length >= 3);
}

function searchByUstensilsV2 (keywords, listReceipts) {
  let result = [];
